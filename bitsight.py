...
# function to decide whether a number is out of range.
def isOutScope(x):
    if 100 <= x <= 500:
        return 0
    else:
...
textfile = sys.argv[1]                  # read in files which is processed..
    ...
    for line in fp.readlines():
        ...
        if failure > 0:                 # If there is a failed number, exit immediately
            ...
            if count > 1:               # number in current line(bufferLines[count]) substract
                ...
                print("%4d\t%4d\t%.2f%%\t (%s %% difference between %4d ->%4d)" % (count, int(number2[1]), (int(number2[1])-int(number1[1]))/int(number1[1])*100, result, int(number1[1]), int(number2[1]) ))
